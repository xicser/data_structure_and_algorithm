...
#define _CRT_SECURE_NO_WARNINGS//scanf加上这个宏，才能编译通过
...
void CreateBiTreeRecursion(BiTree *T)
{
	...
	scanf("%c", &ch);
	if (ch == '#')(*T) = NULL;//空树，保证是叶子结点，递归结束条件
	else{
		(*T) = (BiTNode *)malloc(sizeof(BiTNode));
		if ((*T) == NULL)return;
		(*T)->data = ch;			//生成结点
		CreateBiTreeRecursion(&(*T)->lchild);//构造左子树
		CreateBiTreeRecursion(&(*T)->rchild);//构造右子树
	}
}
